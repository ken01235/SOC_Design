...
//------------------------Parameter----------------------
`define TV_IN_pn32HPInput "../tv/cdatafile/c.fir_n11_maxi.autotvin_pn32HPInput.dat"
`define TV_IN_pn32HPOutput "../tv/cdatafile/c.fir_n11_maxi.autotvin_pn32HPOutput.dat"
`define TV_IN_an32Coef "../tv/cdatafile/c.fir_n11_maxi.autotvin_an32Coef.dat"
`define TV_IN_regXferLeng "../tv/cdatafile/c.fir_n11_maxi.autotvin_regXferLeng.dat"
parameter ADDR_WIDTH = 7;
parameter DATA_WIDTH = 32;
parameter pn32HPInput_DEPTH = 1;
reg [31 : 0] pn32HPInput_OPERATE_DEPTH = 0;
parameter pn32HPInput_c_bitwidth = 64;
parameter pn32HPOutput_DEPTH = 1;
reg [31 : 0] pn32HPOutput_OPERATE_DEPTH = 0;
parameter pn32HPOutput_c_bitwidth = 64;
parameter an32Coef_DEPTH = 12;
reg [31 : 0] an32Coef_OPERATE_DEPTH = 0;
parameter an32Coef_c_bitwidth = 32;
parameter regXferLeng_DEPTH = 1;
reg [31 : 0] regXferLeng_OPERATE_DEPTH = 0;
parameter regXferLeng_c_bitwidth = 32;
parameter pn32HPInput_data_in_addr = 16;
parameter pn32HPOutput_data_in_addr = 28;
parameter regXferLeng_data_in_addr = 40;
...
reg  BREADY_reg = 0;
reg [pn32HPInput_c_bitwidth - 1 : 0] mem_pn32HPInput [pn32HPInput_DEPTH - 1 : 0] = '{default : 'h0};
reg [DATA_WIDTH-1 : 0] image_mem_pn32HPInput [ (pn32HPInput_c_bitwidth+DATA_WIDTH-1)/DATA_WIDTH * pn32HPInput_DEPTH -1 : 0] = '{default : 'hz};
reg pn32HPInput_write_data_finish;
reg [pn32HPOutput_c_bitwidth - 1 : 0] mem_pn32HPOutput [pn32HPOutput_DEPTH - 1 : 0] = '{default : 'h0};
reg [DATA_WIDTH-1 : 0] image_mem_pn32HPOutput [ (pn32HPOutput_c_bitwidth+DATA_WIDTH-1)/DATA_WIDTH * pn32HPOutput_DEPTH -1 : 0] = '{default : 'hz};
reg pn32HPOutput_write_data_finish;
reg [DATA_WIDTH - 1 : 0] mem_an32Coef [an32Coef_DEPTH - 1 : 0] = '{default : 'h0};
reg [DATA_WIDTH-1 : 0] image_mem_an32Coef [ (an32Coef_c_bitwidth+DATA_WIDTH-1)/DATA_WIDTH * an32Coef_DEPTH -1 : 0] = '{default : 'hz};
reg an32Coef_write_data_finish;
reg [DATA_WIDTH - 1 : 0] mem_regXferLeng [regXferLeng_DEPTH - 1 : 0] = '{default : 'h0};
reg [DATA_WIDTH-1 : 0] image_mem_regXferLeng [ (regXferLeng_c_bitwidth+DATA_WIDTH-1)/DATA_WIDTH * regXferLeng_DEPTH -1 : 0] = '{default : 'hz};
reg regXferLeng_write_data_finish;
...
//write regXferLeng reg
reg [31 : 0] write_regXferLeng_count = 0;
reg write_regXferLeng_run_flag = 0;
reg write_one_regXferLeng_data_done = 0;
...
assign TRAN_control_write_data_finish = 1 & pn32HPInput_write_data_finish & pn32HPOutput_write_data_finish & an32Coef_write_data_finish & regXferLeng_write_data_finish;
            ...
            if (write_pn32HPInput_run_flag === 1 && get_ack === 1) begin
                ...
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    if (pn32HPInput_c_bitwidth < 32) begin
                        pn32HPInput_data_tmp_reg = mem_pn32HPInput[write_pn32HPInput_count];
                    end
                    else begin
                        for (j=0 ; j<32 ; j = j + 1) begin
                            if (i*32 + j < pn32HPInput_c_bitwidth) begin
                                pn32HPInput_data_tmp_reg[j] = mem_pn32HPInput[write_pn32HPInput_count][i*32 + j];
                            end
                            else begin
                    ...
                    if(image_mem_pn32HPInput[write_pn32HPInput_count * four_byte_num  + i]!==pn32HPInput_data_tmp_reg) begin
                    ...
                    image_mem_pn32HPInput[write_pn32HPInput_count * four_byte_num + i]=pn32HPInput_data_tmp_reg;
                    end
                end
                process_busy = 0;
            ...
            if (write_pn32HPOutput_run_flag === 1 && get_ack === 1) begin
                ...
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    if (pn32HPOutput_c_bitwidth < 32) begin
                        pn32HPOutput_data_tmp_reg = mem_pn32HPOutput[write_pn32HPOutput_count];
                    end
                    else begin
                        for (j=0 ; j<32 ; j = j + 1) begin
                            if (i*32 + j < pn32HPOutput_c_bitwidth) begin
                                pn32HPOutput_data_tmp_reg[j] = mem_pn32HPOutput[write_pn32HPOutput_count][i*32 + j];
                            end
                            else begin
                    ...
                    if(image_mem_pn32HPOutput[write_pn32HPOutput_count * four_byte_num  + i]!==pn32HPOutput_data_tmp_reg) begin
                    ...
                    image_mem_pn32HPOutput[write_pn32HPOutput_count * four_byte_num + i]=pn32HPOutput_data_tmp_reg;
                    end
                end
                process_busy = 0;
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            ...
            if (write_an32Coef_run_flag === 1 && get_ack === 1) begin
                ...
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    ...
                    if(image_mem_an32Coef[write_an32Coef_count * four_byte_num  + i]!==an32Coef_data_tmp_reg) begin
                    write (an32Coef_data_in_addr + write_an32Coef_count * four_byte_num * 4 + i * 4, an32Coef_data_tmp_reg, write_an32Coef_resp);
                    image_mem_an32Coef[write_an32Coef_count * four_byte_num + i]=an32Coef_data_tmp_reg;
                    end
                end
                process_busy = 0;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        regXferLeng_write_data_finish <= 0;
        write_regXferLeng_run_flag <= 0; 
        write_regXferLeng_count = 0;
        count_operate_depth_by_bitwidth_and_depth (regXferLeng_c_bitwidth, regXferLeng_DEPTH, regXferLeng_OPERATE_DEPTH);
    end
    else begin
        if (TRAN_control_start_in === 1) begin
            regXferLeng_write_data_finish <= 0;
        ...
        if (AESL_ready_reg === 1) begin
            write_regXferLeng_run_flag <= 1; 
            write_regXferLeng_count = 0;
        end
        if (write_one_regXferLeng_data_done === 1) begin
            write_regXferLeng_count = write_regXferLeng_count + 1;
            if (write_regXferLeng_count == regXferLeng_OPERATE_DEPTH) begin
                write_regXferLeng_run_flag <= 0; 
                regXferLeng_write_data_finish <= 1;
            end
        end
    end
end

initial begin : write_regXferLeng
    integer write_regXferLeng_resp;
    integer process_num ;
    integer get_ack;
    integer four_byte_num;
    integer c_bitwidth;
    integer i;
    integer j;
    reg [31 : 0] regXferLeng_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = regXferLeng_c_bitwidth;
    process_num = 3;
    count_c_data_four_byte_num_by_bitwidth (c_bitwidth , four_byte_num) ;
    while (1) begin
        process_3_finish <= 0;
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            get_ack = 1;
            if (write_regXferLeng_run_flag === 1 && get_ack === 1) begin
                process_busy = 1;
                //write regXferLeng data 
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    if (regXferLeng_c_bitwidth < 32) begin
                        regXferLeng_data_tmp_reg = mem_regXferLeng[write_regXferLeng_count];
                    end
                    else begin
                        for (j=0 ; j<32 ; j = j + 1) begin
                            if (i*32 + j < regXferLeng_c_bitwidth) begin
                                regXferLeng_data_tmp_reg[j] = mem_regXferLeng[write_regXferLeng_count][i*32 + j];
                            end
                            else begin
                                regXferLeng_data_tmp_reg[j] = 0;
                            end
                        end
                    end
                    if(image_mem_regXferLeng[write_regXferLeng_count * four_byte_num  + i]!==regXferLeng_data_tmp_reg) begin
                    write (regXferLeng_data_in_addr + write_regXferLeng_count * four_byte_num * 4 + i * 4, regXferLeng_data_tmp_reg, write_regXferLeng_resp);
                    image_mem_regXferLeng[write_regXferLeng_count * four_byte_num + i]=regXferLeng_data_tmp_reg;
                    end
                end
                process_busy = 0;
                write_one_regXferLeng_data_done <= 1;
                @(posedge clk);
                write_one_regXferLeng_data_done <= 0;
            end   
            process_3_finish <= 1;
        end
        @(posedge clk);
...
// Read data from file 
initial begin : read_pn32HPInput_file_process 
  integer fp; 
  integer ret; 
  integer factor; 
  reg [151 : 0] token; 
  reg [151 : 0] token_tmp; 
  //reg [pn32HPInput_c_bitwidth - 1 : 0] token_tmp; 
  ...
  count_seperate_factor_by_bitwidth (pn32HPInput_c_bitwidth , factor);
  fp = $fopen(`TV_IN_pn32HPInput ,"r"); 
  if(fp == 0) begin                               // Failed to open file 
      $display("Failed to open file \"%s\"!", `TV_IN_pn32HPInput); 
      $finish; 
  end 
  read_token(fp, token); 
  ...
  while (token != "[[[/runtime]]]") begin 
      ...
      for(i = 0; i < pn32HPInput_DEPTH; i = i + 1) begin 
          read_token(fp, token); 
          ...
          if (factor == 4) begin
              ...
              if (i%factor == 3) begin
                  ...
                  mem_pn32HPInput [i/factor] = tmp_cache_mem;
                  tmp_cache_mem [DATA_WIDTH - 1 : 0] = 0;
              end
          end
          if (factor == 2) begin
...
// Read data from file 
initial begin : read_pn32HPOutput_file_process 
  integer fp; 
  integer ret; 
  integer factor; 
  reg [151 : 0] token; 
  reg [151 : 0] token_tmp; 
  //reg [pn32HPOutput_c_bitwidth - 1 : 0] token_tmp; 
  reg [DATA_WIDTH - 1 : 0] tmp_cache_mem; 
  reg [ 8*5 : 1] str;
    reg [63:0] trans_depth;
  integer transaction_idx; 
  ...
  transaction_idx = 0; 
  tmp_cache_mem [DATA_WIDTH - 1 : 0] = 0;
  count_seperate_factor_by_bitwidth (pn32HPOutput_c_bitwidth , factor);
  fp = $fopen(`TV_IN_pn32HPOutput ,"r"); 
  if(fp == 0) begin                               // Failed to open file 
      $display("Failed to open file \"%s\"!", `TV_IN_pn32HPOutput); 
      $finish; 
  end 
  read_token(fp, token); 
  if (token != "[[[runtime]]]") begin             // Illegal format 
  ...
  while (token != "[[[/runtime]]]") begin 
      ...
      for(i = 0; i < pn32HPOutput_DEPTH; i = i + 1) begin 
          read_token(fp, token); 
          ret = $sscanf(token, "0x%x", token_tmp); 
          if (factor == 4) begin
              if (i%factor == 0) begin
              ...
              if (i%factor == 3) begin
                  tmp_cache_mem [31 : 24] = token_tmp;
                  mem_pn32HPOutput [i/factor] = tmp_cache_mem;
                  tmp_cache_mem [DATA_WIDTH - 1 : 0] = 0;
              end
          end
          if (factor == 2) begin
...
task write_binary_pn32HPOutput;
    input integer fp;
    input reg[64-1:0] in;
    input integer in_bw;
    reg [63:0] tmp_long;
      ...
      for(i = 0; i < an32Coef_DEPTH; i = i + 1) begin 
         $fread(token_tmp,fp);
          if (factor == 4) begin
              ...
              if (i%factor == 3) begin
                  tmp_cache_mem [31 : 24] = token_tmp;
                  mem_an32Coef [i/factor] = tmp_cache_mem;
                  tmp_cache_mem [DATA_WIDTH - 1 : 0] = 0;
              end
          end
          if (factor == 2) begin
...
// Read data from file 
initial begin : read_regXferLeng_file_process 
  integer fp; 
  integer ret; 
  integer factor; 
  reg [127 : 0] token; 
  reg [127 : 0] token_tmp; 
  //reg [regXferLeng_c_bitwidth - 1 : 0] token_tmp; 
  reg [DATA_WIDTH - 1 : 0] tmp_cache_mem; 
  reg [ 8*5 : 1] str;
    reg [63:0] trans_depth;
  integer transaction_idx; 
  ...
  transaction_idx = 0; 
  tmp_cache_mem [DATA_WIDTH - 1 : 0] = 0;
  count_seperate_factor_by_bitwidth (regXferLeng_c_bitwidth , factor);
  fp = $fopen(`TV_IN_regXferLeng ,"r"); 
  if(fp == 0) begin                               // Failed to open file 
      $display("Failed to open file \"%s\"!", `TV_IN_regXferLeng); 
      $finish; 
  end 
  read_token(fp, token); 
  if (token != "[[[runtime]]]") begin             // Illegal format 
  ...
  while (token != "[[[/runtime]]]") begin 
      ...
      for(i = 0; i < regXferLeng_DEPTH; i = i + 1) begin 
          read_token(fp, token); 
          ret = $sscanf(token, "0x%x", token_tmp); 
          if (factor == 4) begin
              if (i%factor == 0) begin
              ...
              if (i%factor == 3) begin
                  tmp_cache_mem [31 : 24] = token_tmp;
                  mem_regXferLeng [i/factor] = tmp_cache_mem;
                  tmp_cache_mem [DATA_WIDTH - 1 : 0] = 0;
              end
          end
          if (factor == 2) begin
